...
class SinusoidalNoisyExogenousProvider(ExogenousProvider):
    ...
    def _compute_noisy_timeserie(self, timeserie, length, repeat_noise = False, current_timestamp=1):
        ...
        if repeat_noise:
            ...
            sample = length 
        x = np.arange(sample) 
        noise = np.asarray(np.random.normal(size=sample)) 
        y = ((np.sin(x))+noise)
        min_y = -1
        max_y = 1
        y = (2*((y - min_y) / ( max_y - min_y )) - 1) * self._error
        #print(y)
        timeserie_noised = None
        if repeat_noise:
            timeserie_noised = list(flatten([list((1-y[i])*np.array(timeserie_chunks[i])) for i in np.arange(sample)]))
        else:
            timeserie_noised = list((1-y)*timeserie)
