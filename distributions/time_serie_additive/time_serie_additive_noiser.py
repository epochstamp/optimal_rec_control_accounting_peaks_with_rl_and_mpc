...
class TimeSerieAdditiveNoiser(ProbabilityDistribution):
    ...
    def support(self) -> Tuple[float, float]:
        """
            ...
            Returns:
        """
        return (0.0, max(self._initial_time_serie)*(1+self._max_error_scale))
    ...
    def sample(self, length=1) -> Union[List, Any]:
        ...
        noise = np.clip(noise, *self._support_noise())
        noise = normalize_bounds(noise, -1.0, 1.0, self._min_noise, self._max_noise, convert_to_float=False)
        noise = self._post_process_noise(noise)
        min_value, max_value = self.support()
        return np.clip(self._initial_time_serie + (1+noise)*max(self._initial_time_serie), min_value, max_value)
