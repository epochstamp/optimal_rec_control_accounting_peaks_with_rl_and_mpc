...
class SumMinMaxMeters(SpaceConverter):
    ...
    def _convert_observation_space(self):
        if type(self._original_observation_space) != DictSpace:
            raise BaseException(f"Only DictSpace type {type(self._observation_space)} is handled for SumMinMaxNetMeters")
        ...
        return DictSpace({
            ...
            **{
                (member, type_meter): Box(-10000000, 10000000) for member in self._members for type_meter in ("sum_meter", "min_meter", "max_meter")
            }
        })
    ...
    def convert_observation(self,
        ...
        if type(self._current_observation_space) != DictSpace:
            raise BaseException(f"Only DictSpace type {type(self._observation_space)} is handled for CurrMinMaxNetMeters")
        if backward:
            ...
            d_observation = {
                k:v for k,v in d_observation.items() if len(k) < 2 or k[1] not in ("sum_meter", "min_meter", "max_meter")
            }
            ...
            for member in self._members:
                ...
                d_observation[(member, "sum_meter")] = sum(d_observation[(member, "net_meters")])
