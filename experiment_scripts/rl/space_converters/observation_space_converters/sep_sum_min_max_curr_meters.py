...
class SepSumMinMaxCurrMeters(SpaceConverter):
    ...
    def _convert_observation_space(self):
        ...
        return DictSpace({
            ...
            **{
                (member, type_meter): Box(-10000000, 10000000) for member in self._members for type_meter in ("sum_cons_meter", "sum_prod_meter", "min_meter", "max_meter", "curr_meter")
            }
        })
    ...
    def convert_observation(self,
        ...
        if backward:
            ...
            d_observation = {
                k:v for k,v in d_observation.items() if len(k) < 2 or k[1] not in ("sum_cons_meter", "sum_prod_meter", "min_meter", "max_meter", "curr_meter")
            }
            ...
            for member in self._members:
                ...
                d_observation[(member, "sum_cons_meter")] = sum([e for e in d_observation[(member, "net_meters")] if e >= 0][:-1])
                d_observation[(member, "sum_prod_meter")] = sum([e for e in d_observation[(member, "net_meters")] if e <= 0][:-1])
                d_observation[(member, "curr_meter")] = d_observation[(member, "net_meters")][-1]
