...
class CurrMinMaxMeters(SpaceConverter):
    ...
    def _convert_observation_space(self):
        ...
        return DictSpace({
            ...
            **{
                (member, type_meter): Box(-10000000, 10000000) for member in self._members for type_meter in ("curr_meter", "min_meter", "max_meter")
            }
        })
    ...
    def convert_observation(self,
        ...
        if backward:
            ...
            d_observation = {
                k:v for k,v in d_observation.items() if len(k) < 2 or k[1] not in ("curr_meter", "min_meter, max_meter")
            }
