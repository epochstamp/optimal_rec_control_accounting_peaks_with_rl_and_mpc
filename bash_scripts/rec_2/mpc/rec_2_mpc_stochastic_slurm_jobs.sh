...
--wandb-project mpc_study_rec_2_stochastic --random-seed 1436 --n-samples 128 --nb-random-seed $NB_RANDOM_SEED $ENVIRONMENTS $FORECASTS --Delta-M 4 $DELTA_P --Delta-P-prime 0 --n-cpus 1 $MPC_POLICIES $GAMMAS $GAMMAS_POLICIES $RESCALE_GAMMAS --min-K 1 --max-K 101 --step-K 1 \
--max-n-jobs 1 --id-job rec_2_stochastic_mpc --mem-per-cpu 2000 --root-dir $ROOT_DIR --job-dir-name rec_2_stochastic_mpc --time-limits 0 0 45 0 --multiprice-flags no-multiprice $SMALL_PEN_CTRL_ACTIONS $SOLUTION_CHAINED_OPTIMISATION_FLAGS --solver cplex